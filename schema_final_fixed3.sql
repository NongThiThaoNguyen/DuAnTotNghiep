...
ALTER TABLE [user_profiles] ADD [timezone] nvarchar(50) NULL DEFAULT N'Asia/Ho_Chi_Minh';
                ...
                EXEC('UPDATE up SET up.avatar_url = u.avatar_url, up.phone = u.phone FROM user_profiles up INNER JOIN users u ON up.user_id = u.id;');
...
CREATE TABLE [user_settings] (
    ...
    CONSTRAINT [FK_user_settings_users_UserId] FOREIGN KEY ([UserId]) REFERENCES [users] ([id]) ON DELETE CASCADE
                ...
                EXEC('UPDATE u SET u.avatar_url = up.avatar_url, u.phone = up.phone FROM users u INNER JOIN user_profiles up ON u.id = up.user_id;');
...
EXEC('UPDATE [users] SET [avatar_url] = NULL, [phone] = NULL WHERE [id] = 1;');
SELECT @@ROWCOUNT;
...
ALTER TABLE [student_learning_paths] ADD [archived_at] datetime NULL;

ALTER TABLE [student_learning_paths] ADD [path_version] int NOT NULL DEFAULT 1;

ALTER TABLE [student_learning_paths] ADD [replaced_by_path_id] int NULL;

ALTER TABLE [reference_sources] ADD [author] nvarchar(255) NULL;

ALTER TABLE [reference_sources] ADD [compliance_evidence_url] varchar(1000) NULL;
...
ALTER TABLE [learning_path_nodes] ADD [required_node_id] int NULL;

ALTER TABLE [competency_skill_scores] ADD [TopicId] int NULL;

ALTER TABLE [competency_analyses] ADD [IsLatest] bit NOT NULL DEFAULT CAST(0 AS bit);

ALTER TABLE [competency_analyses] ADD [KnowledgeGapsJson] nvarchar(max) NULL;

ALTER TABLE [competency_analyses] ADD [MetadataJson] nvarchar(max) NULL;

ALTER TABLE [competency_analyses] ADD [PrioritizedTopicsJson] nvarchar(max) NULL;

ALTER TABLE [competency_analyses] ADD [Status] nvarchar(max) NOT NULL DEFAULT N'';

ALTER TABLE [ai_usage_logs] ADD [DurationMs] int NULL;

ALTER TABLE [ai_usage_logs] ADD [LatencyMs] int NULL;

ALTER TABLE [ai_usage_logs] ADD [PromptInput] nvarchar(max) NULL;

ALTER TABLE [ai_usage_logs] ADD [ResponseOutput] nvarchar(max) NULL;

ALTER TABLE [ai_prompt_templates] ADD [UserPromptTemplate] nvarchar(max) NULL;

ALTER TABLE [ai_generated_contents] ADD [BatchId] nvarchar(max) NULL;

ALTER TABLE [ai_generated_contents] ADD [PublishedQuestionId] int NULL;

CREATE TABLE [ReplanningRules] (
    [Id] int NOT NULL IDENTITY,
    [LowScoreThreshold] decimal(18,2) NOT NULL,
    [MissedDaysThreshold] int NOT NULL,
    [FastProgressScoreThreshold] decimal(18,2) NOT NULL,
    [AutoApplyEnabled] bit NOT NULL,
    [SuggestionExpiryDays] int NOT NULL,
    [DebounceHours] int NOT NULL,
    [IsActive] bit NOT NULL,
    [CreatedAt] datetime2 NOT NULL,
    [UpdatedAt] datetime2 NULL,
    CONSTRAINT [PK_ReplanningRules] PRIMARY KEY ([Id])
);

CREATE INDEX [IX_student_learning_paths_replaced_by_path_id] ON [student_learning_paths] ([replaced_by_path_id]);

CREATE INDEX [IX_reference_sources_rejected_by] ON [reference_sources] ([rejected_by]);
...
CREATE INDEX [IX_learning_path_nodes_required_node_id] ON [learning_path_nodes] ([required_node_id]);

CREATE INDEX [IX_competency_skill_scores_TopicId] ON [competency_skill_scores] ([TopicId]);

ALTER TABLE [competency_skill_scores] ADD CONSTRAINT [FK_competency_skill_scores_learning_topics_TopicId] FOREIGN KEY ([TopicId]) REFERENCES [learning_topics] ([id]);

ALTER TABLE [learning_path_nodes] ADD CONSTRAINT [FK_learning_path_nodes_required_node] FOREIGN KEY ([required_node_id]) REFERENCES [learning_path_nodes] ([id]);
...
ALTER TABLE [student_learning_paths] ADD CONSTRAINT [FK_student_learning_paths_replaced_by_path] FOREIGN KEY ([replaced_by_path_id]) REFERENCES [student_learning_paths] ([id]);

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20260629173926_SyncModel', N'10.0.9');
